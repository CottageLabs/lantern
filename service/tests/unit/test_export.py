...
class TestImport(testindex.ESTestCase):
    ...
    def test_01_export(self):
        ...
        assert len(rows) == 4
        assert rows[0] == ['PMCID', 'PMID', 'DOI', "ISSN", 'Article title', "Fulltext in EPMC?", 'XML Fulltext?', 'AAM?', 'Open Access?', 'Licence', 'Licence Source', 'Journal Type', 'Correct Article Confidence', 'Standard Compliance?', 'Deluxe Compliance?', 'Notes']
